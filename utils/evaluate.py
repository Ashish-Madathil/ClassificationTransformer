...
def compute_metrics(args,all_predictions,all_targets,all_masks,loss,loss_unk,elapsed,known_labels=0,all_metrics=False,verbose=True):
    ...
    if known_labels > 0:
        ...
        # meanAP = custom_mean_avg_precision(all_targets, all_predictions, unknown_label_mask_threshold=0.5)
    ...
    for label_idx in range(all_targets.shape[1]):
        ...
        label_predictions = all_predictions_top3[:, label_idx]
        
        CP_top3_i = metrics.precision_score(label_targets, label_predictions, average='macro')
        CR_top3_i = metrics.recall_score(label_targets, label_predictions, average='macro')
        OP_top3_i = metrics.precision_score(label_targets, label_predictions, average='micro')
        OR_top3_i = metrics.recall_score(label_targets, label_predictions, average='micro')                        
        CP_top3s.append(CP_top3_i)
        CR_top3s.append(CR_top3_i)
        OP_top3s.append(OP_top3_i)  
        OR_top3s.append(OR_top3_i)                      
    CR_top3 = np.mean(CR_top3s)        
    CP_top3 = np.mean(CP_top3s)
    OP_top3 = np.mean(OP_top3s)    
    OR_top3 = np.mean(OR_top3s)    
    ...
    for label_idx in range(all_targets.shape[1]):
        label_targets = all_targets[:, label_idx]
        label_predictions = all_predictions_thresh[:, label_idx]
        
        CP_i = metrics.precision_score(label_targets, label_predictions, average='macro')
        CR_i = metrics.recall_score(label_targets, label_predictions, average='macro')
        OP_i = metrics.precision_score(label_targets, label_predictions, average='micro')
        OR_i = metrics.recall_score(label_targets, label_predictions, average='micro')                
        
        # Calculate TP and  Accuracy
        TP_i = np.sum((label_targets == 1) & (label_predictions == 1))
        TPs.append(TP_i)
        accuracy_i = np.mean(label_targets == label_predictions)
    ...
    # Calculate label-wise TP and Accuracy
    label_wise_TP = np.array(TPs)
    label_wise_accuracy = np.array(accuracies)
    # Calculate overall TP and Accuracy   
    overall_TP = np.sum(label_wise_TP)
    overall_accuracy = np.mean(label_wise_accuracy) 
    ...
    if verbose:
        ...
        print('Label-wise True Positives:', label_wise_TP)
        print('Label-wise Accuracy:', label_wise_accuracy)
        print('Overall True Positives:', overall_TP)
    ...
    metrics_dict['label_wise_TP'] = label_wise_TP
    metrics_dict['label_wise_accuracy'] = label_wise_accuracy
    metrics_dict['overall_TP'] = overall_TP
    metrics_dict['overall_accuracy'] = overall_accuracy    
