...
def get_unk_mask_indices(image,testing,num_labels,known_labels,epoch=1):
    if testing:
        ...
        # valid_indices = list(range(num_labels))  # Valid indices for the mask tensor
        # known_indices = random.sample(valid_indices, num_known)
        # unk_mask_indices = [idx for idx in valid_indices if idx not in known_indices]        
